	...
	try {
		...
		if (data.status != 200) {
			...
			tableData = content.filter((element) => {
				...
				return element.Project_Name.includes("Plantation");
			});
...
function filterTable({target}) {
	...
	const searchedData = tableData.filter((v)=>{
		return v.Assembly.toLowerCase().includes(the_value) || v.Region.toLowerCase().includes(the_value) || v.Status.toLowerCase().includes(the_value);
	})
